...
![center h:300](merge_subtrees.svg)
...
回答 $m$ 个询问。第 $i$ 个询问给你两个整数 $u_i, c_i$，问
...
求 DFS 序号。对每个点 $u$，定义
- $L_u$：点 $u$ 的 DFS 序号
- $R_u$：子树 $u$ 里最后一个被 DFS 访问的点的 DFS 序号。
...
- 子树 $v_i$ 里，深度等于 $h_i$ 的点上的那些字符，可任意排列，能否构成一个回文串？
...
方法一：对每个深度 $d$，定义序列 $(s_{d,i})_{0 \le i \le |V_d|}$（$|V_d|$ 表示列表 $V_d$ 的长度）
- $s_{d,i}$：一个长为 $26$ 的 01 序列，表示 $V_d$ 里的前 $i$ 个点上，每种字符出现次数的奇偶性。


方法二：定义序列 $(s_i)_{1 \le i \le n}$
- 设 DFS 序号等于 $i$ 的点是 $v$，点 $v$ 的深度是 $d$。
- $s_i$：一个长为 $26$ 的 01 序列，表示在深度等于 $d$ 且 DFS 序号不超过 $i$ 的那些点上，每种字符出现次数的奇偶性。
...
设某人从 $s$ 出发跑向 $t$。我们把路径 $s \leadsto t$ 上的点分成两段
- 上行：从 $s$ 到 $\lca(s, t)$
...
![bg fit right:31%](在差分序列上操作.svg)
设 $u, v$ 是树上两点，$v$ 是 $u$ 的祖先。
向路径 $u \leadsto v$ 上每个点 $j$ 的集合 $A_j$ 里添加一个数 $x$，在差分序列上的效果就是
- 在点 $u$ 处加一个「添加一个 $x$ 」的操作，
- 在点 $v$ 的父节点处加一个「删除一个 $x$」的操作。

然后，对每个点 $j$
- 求集合 $A_j$ 就化为对子树 $j$（里的操作）求和，
...
![bg right:35% fit](把子树信息汇总到全局数据结构.svg)
