...
def gen_test(in_file, test_id):
    ...
    if test_id < 5:
        ...
        return
    if test_id == 5:
        gen_random(f, maxn, maxn)
        return
    
    n = maxn
    q = maxn
    chain = random.uniform(0, 0.1)
    flower = random.uniform(0.6, 0.9)
    tree = Graph.tree(n, chain, flower)
