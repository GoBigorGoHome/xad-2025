...
- 一开始我们在根节点。
- 我们把文本串 $T$ “喂给” AC 自动机，每次喂一个字符，根据此字符在 AC 自动机上跑。
...
1. 构建 $M$ 个模式串的 AC 自动机。
2. 在 AC 自动机上跑每个文本串，算出字典树的**每个节点被匹配到的次数**。
3. 利用后缀链接反向传播，算出字典树的**每个节点在 $N$ 个文本串里出现的次数**。
4. 输出每个单词节点（也就是每个模式串）在 $N$ 个文本串里出现的次数。
